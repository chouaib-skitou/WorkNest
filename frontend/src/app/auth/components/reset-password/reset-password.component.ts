...
export class ResetPasswordComponent implements OnInit {
  ...
  hasPasswordMismatch(): boolean {
    const confirmPasswordTouched = !!this.resetForm.get("confirmNewPassword")?.touched
    const newPasswordTouched = !!this.resetForm.get("newPassword")?.touched
    return !!this.resetForm.hasError("mismatch") && confirmPasswordTouched && newPasswordTouched
  }
        ...
        this.authService.resetPassword(this.token, newPassword, confirmNewPassword).subscribe({
          ...
          error: (error: any) => {
            ...
            if (error.originalError?.error?.errors && Array.isArray(error.originalError.error.errors)) {
              ...
              this.message = error.message || "Failed to reset password. Please try again."
            }
