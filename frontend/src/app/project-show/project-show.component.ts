...
export class ProjectShowComponent implements OnInit, AfterViewInit {
  ...
  uploadTaskImages(images: File[]): Observable<string[]> {
    ...
    const uploadObservables = images.map((image) =>
      this.documentService.createDocument(image).pipe(
        map((response: DocumentResponse) => {
          ...
          if (!imageUrl) {
            console.error(
              'Image upload succeeded but URL was not provided',
              response
            );
    ...
    return forkJoin(uploadObservables).pipe(
      map((urls: (string | null)[]) => {
        ...
        console.log(
          'All task images uploaded successfully, final URLs:',
          validUrls
        );
        return validUrls;
      })
  ...
  switchToEditTask(): void {
    console.log(
      'Switching from view to edit mode for task:',
      this.selectedTask
    );
    ...
    if (this.selectedTaskImages.length > 0) {
      ...
      this.taskService.updateTask(this.selectedTask.id, taskData).subscribe({
        ...
        error: (error: HttpErrorResponse) => {
          ...
          this.formError = 'Failed to update task. Please try again.';
          this.flashMessageService.showError(
            'Failed to update task: ' + (error.error?.message || error.message)
          );
          this.formSubmitting = false;
        },
      });
  ...
  searchDocuments(): void {
    ...
    if (!this.documentSearchQuery) {
      ...
      this.filteredDocuments = this.project.documents.filter(
        (document) =>
  ...
  updatePaginatedDocuments(): void {
    ...
    this.paginatedDocuments = this.filteredDocuments.slice(
      startIndex,
      endIndex
    );
    this.documentTotalPages = Math.ceil(
      this.filteredDocuments.length / this.documentPageSize
    );
  }
  ...
  editDocument(documentUrl: string): void {
    ...
    this.flashMessageService.showInfo(
      'Document editing functionality is coming soon'
    );
  }
      ...
      .pipe(
        catchError((error) => {
          ...
          if (error.status === 404) {
            ...
            console.log(
              'Document not found in storage, continuing with project update'
            );
            return of({
              message: 'Document not found, continuing with project update',
            });
          }
      ...
      .subscribe({
        next: () => {
          ...
          if (this.project && this.project.documents) {
            ...
            this.projectService
              .partialUpdateProject(this.projectId, {
              ...
              .subscribe({
                ...
                error: (updateError) => {
                  console.error(
                    'Error updating project after document deletion:',
                    updateError
                  );
                  this.flashMessageService.showError(
      ...
      next: (response: DocumentResponse) => {
        ...
        if (documentUrl && this.project) {
          ...
          try {
            ...
            this.projectService
              ...
              .subscribe({
                next: (updatedProject) => {
                  ...
                  this.flashMessageService.showSuccess(
                    'Document added to project successfully'
                  );
                  this.project = updatedProject;
                ...
                error: (updateError) => {
                  console.error(
                    'Error updating project with new document:',
                    updateError
                  );
                  this.flashMessageService.showError(
                    'Document uploaded but could not be linked to project: ' +
            ...
            this.closeAddDocumentModal();
            this.flashMessageService.showError(
              `Invalid document URL format: ${documentUrl}`
            );
            this.formSubmitting = false;
          }
        } else {
          ...
          this.flashMessageService.showSuccess(
            response?.message ||
              'Document uploaded successfully, but location was not provided'
          );
          this.formSubmitting = false;
        ...
        this.documentService.createDocument(imageFile).subscribe({
          next: (response: DocumentResponse) => {
            ...
            if (imageUrl && this.project) {
                ...
                this.projectService
                  ...
                  .subscribe({
                    ...
                    error: (updateError) => {
                      ...
                      this.flashMessageService.showError(
                        'Image uploaded but could not be set as project image'
                      );
                      this.formSubmitting = false;
                      this.closeEditProjectImageModal();
                    },
              ...
              this.closeEditProjectImageModal();
              this.flashMessageService.showError(
                'Image uploaded but location was not provided'
              );
              this.formSubmitting = false;
            }
          },
