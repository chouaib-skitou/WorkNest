  ...
  describe("🛠 Task Service Tests", () => {
    ...
    describe("getTaskByIdService", () => {
      ...
      test("🚫 rejects with 404 if task not found", async () => {
        ...
        await expect(getTaskByIdService(adminUser, "non-existent-id")).rejects.toEqual({
          status: 404,
    ...
    describe("createTaskService", () => {
      test("🚫 rejects with 403 if user is not allowed to create task", async () => {
        await expect(createTaskService(employeeUser, { title: "Test" })).rejects.toEqual({
          status: 403,
      ...
      test("🚫 rejects with 409 on duplicate title (P2002)", async () => {
        ...
        await expect(
          createTaskService(adminUser, { title: "Duplicate", projectId: "project-uuid" })
        ).rejects.toEqual({
          status: 409,
      ...
      test("🚫 rejects with 500 on generic error in createTaskService", async () => {
        ...
        await expect(
          createTaskService(adminUser, { title: "Test", projectId: "project-uuid" })
        ).rejects.toEqual({
          status: 500,
          message: "Internal server error",
        });
    ...
    describe("updateTaskService", () => {
      beforeEach(() => {
        prisma.task.findUnique.mockResolvedValue(mockTask);
        prisma.task.update.mockResolvedValue({ ...mockTask, title: "updated task", priority: "LOW" });
      });
      ...
      test("✅ allows manager to fully update if they are project.managerId", async () => {
        ...
        const taskManagedByManager = { ...mockTask, Project: projectWhereManagerIsId };
      ...
      test("✅ updates task successfully and converts title to lowercase", async () => {
        ...
        expect(result).toEqual(new TaskDTO({ ...mockTask, title: "updated task", priority: "LOW" }));
      });
    ...
    describe("patchTaskService", () => {
      beforeEach(() => {
        prisma.task.findUnique.mockResolvedValue(mockTask);
        prisma.task.update.mockResolvedValue({ ...mockTask, title: "patched task" });
      });
      test("🚫 rejects with 403 if manager tries to PATCH multiple fields (not just stageId)", async () => {
        ...
        await expect(patchTaskService(managerUser, mockTask.id, multiPatch)).rejects.toEqual({
          status: 403,
          message: "Access denied: You do not have permission to update this task",
        });
      });
      ...
      test("✅ patches task successfully and converts title to lowercase", async () => {
        ...
        expect(result).toEqual(new TaskDTO({ ...mockTask, title: "patched task" }));
      });
      ...
      test("✅ allows manager to PATCH if they are project.managerId", async () => {
        ...
        const taskManagedByManager = { ...mockTask, Project: projectManagedByUser };
        prisma.task.findUnique.mockResolvedValue(taskManagedByManager);
      ...
      test("✅ allows ROLE_MANAGER to patch task if they are only in employeeIds and only stageId is updated", async () => {
        ...
        const result = await patchTaskService(managerUser, mockTask.id, partialData);
      ...
      test("✅ allows ROLE_EMPLOYEE to patch task if only stageId is updated", async () => {
        ...
        const partialData = { stageId: "new-stage-uuid" };
        prisma.task.update.mockResolvedValue({ ...mockTask, stageId: "new-stage-uuid" });
        const result = await patchTaskService(employeeUser, mockTask.id, partialData);
        expect(prisma.task.update).toHaveBeenCalledWith(
          expect.objectContaining({
            where: { id: mockTask.id },
            data: { stageId: "new-stage-uuid" },
        ...
        expect(result).toEqual(new TaskDTO({ ...mockTask, stageId: "new-stage-uuid" }));
      });
  
      test("🚫 rejects for ROLE_EMPLOYEE if patching fields other than stageId", async () => {
        const partialData = { title: "Not Allowed" };
        await expect(patchTaskService(employeeUser, mockTask.id, partialData)).rejects.toEqual({
          status: 403,
          message: "Access denied: Employees can only update the task stage",
        });
      });
      ...
      test("🚫 rejects with 400 if no valid fields provided in patchTaskService", async () => {
        await expect(patchTaskService(adminUser, mockTask.id, {})).rejects.toEqual({
          status: 400,
      ...
      test("🚫 rejects with 409 on duplicate title in patchTaskService", async () => {
        prisma.task.update.mockRejectedValue({ code: "P2002" });
        await expect(patchTaskService(adminUser, mockTask.id, { title: "Duplicate" })).rejects.toEqual({
          status: 409,
          message: "A task with this title already exists for this project",
        });
      });
      ...
      test("🚫 rejects with 500 on generic error in patchTaskService", async () => {
        prisma.task.update.mockRejectedValue(new Error("Database error"));
        await expect(patchTaskService(adminUser, mockTask.id, { title: "Updated Task" })).rejects.toEqual({
          status: 500,
          message: "Internal server error",
        });
      });
    ...
    describe("deleteTaskService", () => {
      test("✅ deletes task successfully (200)", async () => {
        ...
        expect(result).toEqual({ status: 200, message: "Task deleted successfully" });
      });
      ...
      test("🚫 rejects with 403 if unknown role tries to delete", async () => {
        ...
        await expect(deleteTaskService(unknownRoleUser, mockTask.id)).rejects.toEqual({
          status: 403,
          message: "Access denied: You do not have permission to delete this task",
        });
      });
      ...
      test("🚫 rejects with 404 if task not found in deleteTaskService", async () => {
        prisma.task.findUnique.mockResolvedValue(null);
        await expect(deleteTaskService(adminUser, "non-existent-task")).rejects.toEqual({
          status: 404,
          message: "Task not found",
        });
      });
