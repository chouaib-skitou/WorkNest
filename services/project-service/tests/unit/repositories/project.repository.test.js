...
describe("ProjectRepository", () => {
  ...
  describe("findUnique", () => {
    ...
    it("should return null if no record found", async () => {
      ...
      const result = await ProjectRepository.findUnique({
        where: { id: "not-found" },
      });
  ...
  describe("findMany", () => {
    ...
    it("should propagate errors from prisma.project.findMany", async () => {
      ...
      await expect(ProjectRepository.findMany({})).rejects.toThrow(
        "findMany error"
      );
    });
  });
  ...
  describe("create", () => {
    ...
    it("should propagate errors from prisma.project.create", async () => {
      ...
      await expect(ProjectRepository.create({ name: "Fail" })).rejects.toThrow(
        "create error"
      );
    });
  });
  ...
  describe("update", () => {
    ...
    it("should propagate errors from prisma.project.update", async () => {
      ...
      await expect(ProjectRepository.update("id-123", {})).rejects.toThrow(
        "update error"
      );
    });
  });
  ...
  describe("delete", () => {
    ...
    it("should propagate errors from prisma.project.delete", async () => {
      ...
      await expect(ProjectRepository.delete("bad-id")).rejects.toThrow(
        "delete error"
      );
    });
  });
